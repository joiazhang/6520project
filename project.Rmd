...
```{r}
betahats = my_OGD(X=X, Y=Y, lr=0.00001, beta_0=rep(0, ncol(X)))
rowMeans(betahats)
plot(betahats[n, ] - beta)
sum(abs(betahats[n, ] - beta) > 0.5*mean(beta[nonzero_indexes])) == k
mean(betahats[n, nonzero_indexes]) - mean(betahats[n, -nonzero_indexes])
plot(rowMeans(betahats[, nonzero_indexes]) - rowMeans(betahats[, -nonzero_indexes]), xlab="time", ylab="difference", main="difference in beta and betahat for the k non-negative coefficients")
```
