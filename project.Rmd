...
beta_0 = rep(0, ncol(X)) # weight initialization
# beta_0 = runif(ncol(X))
...
```{r}
# Adagrad
temp = my_adagrad(X=X, Y=Y, lr=lr, beta_0=rep(0, p), full=F)
betahats = temp[[1]]
runtimes = temp[[2]]
...
```{r}
# Adagrad
temp = my_adagrad(X=X, Y=Y, lr=lr, beta_0=rep(0, p), full=F, regression=F)
betahats = temp[[1]]
runtimes = temp[[2]]
runtime_df$adagrad = runtimes
